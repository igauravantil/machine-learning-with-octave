...
resultList = zeros(length(cList) + length(sigmaList), 3);
row= 1;
for i = 1:length(cList),
  for j = 1:length(sigmaList),
    ...
    resultList(row,:) = [predictionError(i,j),cTest,sigmaTest];
    row = row+1;
    
  endfor
endfor
finalResult = sortrows(resultList,1);
C= finalResult(1,2);
sigma = finalResult(1,3);
